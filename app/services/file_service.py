...
STORAGE_DIR = Path(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "storage")))
...
def store_file_permanently(temp_file_path: Path, file_hash: str) -> Path:
    ...
    if permanent_file_path.exists():
        logger.info(f"File with hash {file_hash} already exists at {permanent_file_path}")
